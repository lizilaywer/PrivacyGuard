...
VERSION = f"{APP_VERSION} - Release Audit and Final Polish"
...
class WordBatchReplaceWorker(QThread):
        ...
        if system == "Darwin":
            ...
            for candidate in candidates:
                if os.path.exists(candidate):
                    return candidate
        return shutil_module.which("soffice") or "soffice"

    def _convert_with_libreoffice(self, doc_path):
        import subprocess
        import tempfile

        temp_dir = tempfile.mkdtemp(prefix="pg_batch_doc_")
        ...
        if not os.path.exists(docx_path):
            raise ConversionError("LibreOffice 转换失败", "未生成 .docx 输出文件")
        return docx_path
    ...
    def _convert_with_antiword(self, doc_path):
        ...
        temp_dir = tempfile.mkdtemp(prefix="pg_batch_doc_anti_")
        self._temp_dirs.append(temp_dir)
